...
const TYPE_DESCRIPTIONS: Record<DeployableTokenType, string> = {
  ...
  [TokenType.dravanaSynthetic]:
    'Dravana synthetic (DravanaSynthetic.sol): HypERC20 + TokenRouter.transferRemote; inbound pending mint + consumeAndMint',
};
 ...
 * - Puts `synthetic` then `dravanaSynthetic` first so DravanaSynthetic is easy to spot (long lists + inquirer paging).
 */
  ...
  return ordered.map((type) => ({
    name:
      type === TokenType.dravanaSynthetic
        ? 'dravanaSynthetic (DravanaSynthetic.sol)'
        : String(type),
    value: type,
    description:
